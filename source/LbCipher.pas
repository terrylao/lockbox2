...
type
  ...
  TRDLContext = packed record
    ...
    Rounds  : DWord;
    case Byte of
      0 : (W  : array[0..(MaxRDLRounds * 6)] of TRDLVector);//由 *4 改成 *6 因為ROUND 數會超過 * 4 , AES256 就超過56了.
      1 : (Rk : array[0..MaxRDLRounds *2] of TRDLBlock);
    end;
...
procedure EncryptRDLCTR(const Context : TRDLContext;  Nonce : TRDLBlock; var Block : TRDLBlock);
{ MD5 message digest }
...
function GETU32(pt:PByte ):Cardinal ;
begin
result:=((byte(pt[0]) shl 24) or (byte(pt[1]) shl 16) or (byte(pt[2]) shl  8) or byte(pt[3]));
end;

procedure PUTU32(ct:PByte; st:Cardinal );
begin
 ct[0] :=( st shr 24);
 ct[1] :=( st shr 16);
 ct[2] :=( st shr  8);
...
var
	c :LongWord;
begin
	//GETU32(p)以Big Endian 將一個4字節的char buffer的內容轉為int 
...
procedure EncryptRDLCTR(const Context : TRDLContext;  Nonce : TRDLBlock; var Block : TRDLBlock);
{ encrypt/decrypt block CTR mode }
begin
  if Context.Encrypt then begin
    EncryptRDL(Context, Nonce);
    XorMemPrim(Block, Nonce, SizeOf(Block));
  end 
	else 
	begin
    EncryptRDL(Context, Nonce);   
